...
describe('[Challenge] Free Rider', function () {
    ...
    it('Execution', async function () {
        ...
        // 
        const attackContractFactory = await ethers.getContractFactory('FreeRiderAttacker', player);
        const attackContract = await attackContractFactory.deploy(marketplace.address, nft.address, uniswapPair.address, weth.address, devsContract.address);
        await attackContract.connect(player).attack({ value: ethers.utils.parseEther("0.1") }); // Fee to pay for the flash swap
    });
